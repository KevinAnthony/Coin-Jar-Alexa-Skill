...
var CoinJar = function() { AlexaSkill.call(this, APP_ID); };
...
CoinJar.prototype.eventHandlers.onSessionStarted = function(sessionStartedRequest, session) {
    ...
    CoinLocale = require("./locales/" + sessionStartedRequest.locale + "/Coins.js");
    Formatter = new Intl.NumberFormat(sessionStartedRequest.locale, { style: "currency", minimumFractionDigits: 2, });
    I18N.init({
        lng: sessionStartedRequest.locale,
        resGetPath: "locales/__lng__/__ns__.json"
    });
    Jar.CoinLocale = CoinLocale;
    Jar.I18N = I18N;
};
...
CoinJar.prototype.intentHandlers = {
    "AddCoins": function(intent, session, response) { handleAddCoinsRequest(intent, session, response); },
    "RemoveCoins": function(intent, session, response) { handleRemoveCoinsReques(intent, session, response); },
    "RollCoins": function(intent, session, response) { handleRollCoinsRequest(intent, session, response); },
    "CashRolls": function(intent, session, response) { handleCashRollsRequest(intent, session, response); },
    "GetRollCount": function(intent, session, response) { handleGetRollCountRequest(intent, session, response); },
    "GetCoinCount": function(intent, session, response) { handleGetCoinCountRequest(intent, session, response); },
...
function handleAddCoinsRequest(intent, session, speech) {
    if (Jar.Coins.length === 0) {
        Jar.Load(session, speech, () => { addCoinRequest(intent, session, speech); });
    } else {
...
function handleRemoveCoinsRequest(intent, session, speech) {
    if (Jar.Coins.length === 0) {
        Jar.Load(session, speech, () => { removeCoinsRequest(intent, session, speech); });
    } else {
...
function handleRollCoinsRequest(intent, session, speech) {
    if (Jar.Coins.length === 0) {
        Jar.Load(session, speech, () => { rollCoinsRequest(intent, session, speech); });
    } else {
...
function handleCashRollsRequest(intent, session, speech) {
    if (Jar.Coins.length === 0) {
        Jar.Load(session, speech, () => { cashRollsRequest(intent, session, speech); });
    } else {
...
function handleGetRollCountRequest(intent, session, speech) {
    if (Jar.Coins.length === 0) {
        Jar.Load(session, speech, () => { getRollCountRequest(intent, session, speech); });
    } else {
...
function handleGetCoinCountRequest(intent, session, speech) {
    if (Jar.Coins.length === 0) {
        Jar.Load(session, speech, () => { getCoinCountRequest(intent, session, speech); });
    } else {
...
function handleWelcomeRequest(response) { response.tell("I now support coin jars, right now with google drive backing"); }
